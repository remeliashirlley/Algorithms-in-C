/* Write functions, HashInsert() and HashDelete(), to perform insertion and deletion of a closed addressing hash table by using a doubly linked list. */
...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    if(HashSearch(*Q1Ptr, key)!=NULL) //already exists
        return 0;

    if(Q1Ptr->hSize!=0)
        slot = Hash(key,Q1Ptr->hSize);
    else
        return 0;

    //The key is inserted from the front
    ptrx = (ListNode *) malloc(sizeof(ListNode));
    ptrx->key = key;
    ptrx->next= Q1Ptr->Table[slot].head;
    Q1Ptr->Table[slot].head =ptrx;
    ptrx->pre=NULL;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    if(HashSearch(*Q1Ptr, key)==NULL)   //Doesnt exist
        return 0;
    else
        ptrx= HashSearch(*Q1Ptr, key); //exists & label location with ptrx
    
    if(Q1Ptr->hSize!=0)
        slot = Hash(key,Q1Ptr->hSize);
    
    if(ptrx->pre != NULL && ptrx->next != NULL) //only node
    {
        ptrx->pre->next = ptrx->next;
        ptrx->next->pre = ptrx->pre;
    }
    else if(ptrx->pre == NULL && ptrx->next!=NULL) //first node
    {
        Q1Ptr->Table[slot].head = ptrx->next;
        ptrx->next->pre=NULL;
    }
    else if(ptrx->pre!=NULL && ptrx->next == NULL) //last node
    {
        ptrx->pre->next = NULL;
    }
    free(ptrx);
    ...
    if(HashSearch(*Q1Ptr, key)==NULL)   //successfully removed
        return 1;
}
